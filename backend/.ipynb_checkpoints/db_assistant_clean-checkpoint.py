...
Generate ONLY a valid SQL query:"""
            ...
            if "AS PIE CHART" in sql_query.upper():
                chart_type = 'pie'
                sql_query = re.sub(r'\sAS PIE CHART\s', '', sql_query, flags=re.IGNORECASE)
            elif "AS BAR CHART" in sql_query.upper():
                chart_type = 'bar'
                sql_query = re.sub(r'\sAS BAR CHART\s', '', sql_query, flags=re.IGNORECASE)
    ...
    class DatabaseAssistant:
        ...
        def get_response_from_db_assistant(self, query):  # ← 4 spaces indentation
